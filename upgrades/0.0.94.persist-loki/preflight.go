...
import (
	...
	"github.com/oslokommune/okctl-upgrade/upgrades/0.0.94.persist-loki/pkg/lib/commonerrors"
...
func preflight(fs *afero.Afero, clusterManifest v1alpha1.Cluster) error {
	if !clusterManifest.Integrations.Loki {
		return fmt.Errorf("integration disabled: %w", commonerrors.NoActionRequired)
	}
	...
	if awsStorageExists {
		return fmt.Errorf("storage already exists: %w", commonerrors.NoActionRequired)
	}
